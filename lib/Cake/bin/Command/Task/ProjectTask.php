 ...
 */
namespace lib\Cake\Console\Command\Task;
	...
	public function main() {
		...
		if ($this->bake($project)) {
			...
			if ($this->securitySalt($path) === true) {
				...
				$this->err(__d('cake_console', 'Unable to generate random hash for \'Security.salt\', you should change it in {0}', APP . 'Config' . DS . 'core.php'));
				$success = false;
			}
			...
			if ($this->securityCipherSeed($path) === true) {
				...
				$this->err(__d('cake_console', 'Unable to generate random seed for \'Security.cipherSeed\', you should change it in {0}', APP . 'Config' . DS . 'core.php'));
				$success = false;
			}
			...
			if ($success) {
				$this->out(__d('cake_console', ' * CAKE_CORE_INCLUDE_PATH set to {0} in webroot/index.php', CAKE_CORE_INCLUDE_PATH));
				$this->out(__d('cake_console', ' * CAKE_CORE_INCLUDE_PATH set to {0} in webroot/test.php', CAKE_CORE_INCLUDE_PATH));
			} else {
			...
			if (!$Folder->chmod($path . 'tmp', 0777)) {
				...
				$this->out(__d('cake_console', 'chmod -R 0777 {0}', $path . DS . 'tmp'));
				$success = false;
			}
			if ($success) {
		...
		switch (strtolower($looksGood)) {
			case 'y':
				...
				foreach ($Folder->messages() as $message) {
					$this->out(Text::wrap(' * ' . $message), 1, Shell::VERBOSE);
				}
 ...
 */
	public function getPrefix() {
		...
		if ($this->interactive) {
			$this->hr();
			$this->out(__d('cake_console', 'You need to enable Configure::write(\'Routing.prefixes\',array(\'admin\')) in /app/Config/core.php to			$this->out(__d('cake_console', 'What would you like the prefix route to be?'));
			...
			if ($this->cakeAdmin($admin) !== true) {
				...
				$this->out(__d('cake_console', 'You need to enable Configure::write(\'Routing.prefixes\',array(\'admin\')) in /app/Config/core.php to				$this->_stop();
			}
			return $admin . '_';
		}
		return '';
