 ...
 */
	protected function _extractTokens() {
		foreach ($this->_files as $file) {
			$this->_file = $file;
			$this->out(__d('cake_console', 'Processing {0}...', $file));
 ...
 */
	protected function _extractValidationMessages() {
		...
		if (!empty($this->params['plugin'])) {
			/* TODO: App::uses($this->params['plugin'] . 'AppModel', $this->params['plugin'] . '.Model'); */
 ...
 */
	protected function _markerError($file, $line, $marker, $count) {
		$this->out(__d('cake_console', "Invalid marker content in {0}:{1}\n* {2}(", $file, $line, $marker), true);
		$count += 2;
		$tokenCount = count($this->_tokens);
		$parenthesis = 1;
